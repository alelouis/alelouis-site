...
> An arpeggiator can be defined as something that builds a sequence of notes out of a given collection of notes.
 ...
 Let's take a basic chord : **C Major**. 
 
 The **C** major chord is composed of one root note, here **C** (or Do), and two intervals : a major third (**E** or mi) and a perfect fifth (**G** or Sol).

An arpeggiator applied to the chord C Major would create a sequence out of its notes **{C, E, G}**.
The simplest sequence one can think about is the ascending [arpeggio](https://en.wikipedia.org/wiki/Arpeggio) : **C**, then **E**, then **G**.

Of course, imagination is the limit here (and combinatorics too but shh).

## Mumuse

I recently started working on a music theory library in Rust : **mumuse**. The developement **repository** is [available](https://github.com/alelouis/mumuse) on Github, **documentation** [here](https://docs.rs/mumuse/0.5.0/mumuse/) and **download** [here](https://crates.io/crates/mumuse/).

As the time of writing, the library provides building blocks for elementary note, chord and scale manipulations. I often feel stuck in front of my DAW when I want to explore music ideas in a procedural way. Being an **[Ok()](https://doc.rust-lang.org/std/result/)** programmer, I can explore many different ideas as code and often quicker than I would in front of a piano ([which I do too](https://www.youtube.com/watch?v=gnSyoo8W0S0&ab_channel=AlexIsLouis), but with different goals).

As soon as I released an early version of **mumuse**, I wanted to use it in a practical way (other than writing test modules, I mean). Then came the idea of recreating some of the arpeggiator modes in Ableton's default plugin. The constraint I would impose to myself being to do that with only **[iterators](https://doc.rust-lang.org/std/iter/trait.Iterator.html)** and **mumuse**.
...
Well, not going through the horrendous details, an iterator is something that iterates through an iterable, one element at a time. Yes, my definitions today are top notch (really bad and not complete what so ever, take what I say with a grain of salt). 

Consider this list of integer : `[1, 2, 3, 4]`. An iterator would go through all the values independently, in order. What's interesting is that we can, in a functional programming fashion, modify the behavior or effect of an iterator. We can apply **iterators methods**. In **Rust language**, iterators methods can be one of two types : **adapters** or **consumers**. 

**Adapters** methods return other iterators while consumers *consume* the iterator and output values, vectors or other things that are not iterators. 

**E.g.** with a popular adapter method : `map` 

```rust 
let var = (1..4).map(|value| value * 2);
```

The `map` method takes as an argument a **closure** (anonymous function) the will be applied on each value encountered in `var`. In this example, the closure simply multiplies the current value by `2`. Note that the computation isn't actually performed here, `var` isn't equal to `[2, 3, 6, 8]` after this line. Instead, `var` is **lazy evaluated** : each value will be computed only when their are needed in the program.
...
I think you know well enough to go through the next section : **making arpeggiators out of iterators** !
...
The function `get_notes()` use the `Scale` struct in order to retrieve the second (`two`) chord with `n = 4` notes. The field `notes` of the `Chord`, which is of type `Vec<Note>`, is then returned. **All is set up in order to start building arpeggiators !**

### Up

This one is the easiest. The **Up** arpeggiator plays notes bottom up. This is the default behavior we get out of an iterator on a vector (if the vector contains ascending notes, which it does). Because of this, the implementation is straightforward.

```rust
let up = get_notes(n).into_iter();
```
Then, I did two things to appreciate the result.
- First, generating the sheet music through **[music21](https://web.mit.edu/music21/)** and **[LilyPond](http://lilypond.org/)** via a **Tiny Notation** proxy printed from the iterator. 
...
The **Down** iterator goes the other way around. The `notes` consequently need to be iterated backwards. This is precisely what the `.rev()` method does: reverse the order of iteration.
...
The **Up Down** arpeggiator, not to be confused with Up **And** Down, goes up then down but without the last note for the up run and without the first of the down eun. This avoids the repetition of the higher and lower notes. We employ here the `.take(n)` method which only iterates on the first `n` elements. Then we use the `.chain()` method the concatenates the cropped **Up** and **Down** iterators.
...
While the **Up Down** variant was avoiding the higher and lower note repetition, the **Up And Down** doesn't care, so it's even simpler to implement.
...
Same idea, just in the other direction.
...
The **Converge** mode goes from outer notes towards inner notes. From an iterator point of view, we can implement this by **interleaving** the **Up** and **Down** iterators. In addition, we must be taking only the first `n` elements to avoid diverging from mid point.

**Interleaving** is the process of alternating some values with others. 

**E.g.** If you were to interleave `[0, 0, 0, 0]` with `[1, 1, 1, 1]`, you would get `[0, 1, 0, 1, 0, 1, 0, 1]`.
...
Well, **Diverge** goes the other way. Starting from inner notes and expanding towards higher and lower notes. Just before, I said that we avoided diverging by stopping at mid point. That's because by **interleaving** the **Up** and **Down** iterators, some sort of cross pattern is created : the notes first converge and then they diverge from each other. 
...
From this observation, we can simply use the same idea we employed in **Converge**, skip the converge part with `.skip()`, and finally take the next `n` elements that are diverging.
...
Another interesting one and, I must admit, one of my favorites. 

The **Pinky Up** mode emulates your pinky playing the higher note in between an **Up** run. This creates a pedal effect, which I love.

For this one, I use the `.intersperse()` adapter, which is a bit like `.interleave()`, the only difference being that the first one takes as input a single value. This single value is added in-between each element of the original iterator.
...
This was fun. It was also a good exercise for me, and I learned several things :
- I had to analyse arpeggiators modes note by note and I feel like I understand them perfectly now.
- Playing with **[Rust iterators](https://doc.rust-lang.org/std/iter/trait.Iterator.html)** methods, which I will try to (ab)use more in my code style.
